...
%%
%% ```
%% > r3h_hex_api_release:get(r3h_hex_core:default_config(), <<"package">>, <<"1.0.0">>).
%% {ok, {200, ..., #{
...
%% '''
%% @end
-spec get(r3h_hex_core:config(), binary(), binary()) -> r3h_hex_api:response().
get(Config, Name, Version) when is_map(Config) and is_binary(Name) and is_binary(Version) ->
    Path = r3h_hex_api:build_repository_path(Config, ["packages", Name, "releases", Version]),
...
%%
%% ```
%% > r3h_hex_api_release:publish(r3h_hex_core:default_config(), Tarball).
%% {ok, {200, ..., #{
%%      <<"checksum">> => <<"540d210d81f56f17f64309a4896430e727972499b37bd59342dc08d61dff74d8">>,
%%      <<"docs_html_url">> => <<"https://hexdocs.pm/package/1.0.0/">>,
%%      <<"downloads">> => 740,<<"has_docs">> => true,
...
%% '''
%% @end
-spec publish(r3h_hex_core:config(), binary()) -> r3h_hex_api:response().
publish(Config, Tarball) when is_map(Config) and is_binary(Tarball) ->
    Path = r3h_hex_api:build_repository_path(Config, ["publish"]),
    ...
    r3h_hex_api:post(Config2, Path, Body).
...
%%
%% ```
%% > r3h_hex_api_release:delete(r3h_hex_core:default_config(), <<"package">>, <<"1.0.0">>).
%% {ok, {204, ..., nil}}
%% '''
%% @end
-spec delete(r3h_hex_core:config(), binary(), binary()) -> r3h_hex_api:response().
delete(Config, Name, Version) when is_map(Config) and is_binary(Name) and is_binary(Version) ->
    Path = r3h_hex_api:build_repository_path(Config, ["packages", Name, "releases", Version]),
    r3h_hex_api:delete(Config, Path).
...
%%
%% ```
%% > r3h_hex_api_release:retire(r3h_hex_core:default_config(), <<"package">>, <<"1.0.0">>, Params).
%% {ok, {204, ..., nil}}
%% '''
%% @end
-spec retire(r3h_hex_core:config(), binary(), binary(), retirement_params()) -> r3h_hex_api:response().
retire(Config, Name, Version, Params) when is_map(Config) and is_binary(Name) and is_binary(Version) ->
    Path = r3h_hex_api:build_repository_path(Config, ["packages", Name, "releases", Version, "retire"]),
    r3h_hex_api:post(Config, Path, Params).
...
%%
%% ```
%% > r3h_hex_api_release:unretire(r3h_hex_core:default_config(), <<"package">>, <<"1.0.0">>).
%% {ok, {204, ..., nil}}
%% '''
%% @end
-spec unretire(r3h_hex_core:config(), binary(), binary()) -> r3h_hex_api:response().
