...
%% > r3h_hex_tarball:create(Metadata, Files).
%% {ok, #{tarball => <<86,69,...>>,
...
%% > r3h_hex_tarball:create_docs(Files).
%% {ok, <<86,69,...>>}
%% '''
%% @end
...
%% returned from `r3h_hex_repo:get_package(Config, Package)`.
%%
%% Examples:
%%
%% ```
%% > r3h_hex_tarball:unpack(Tarball, memory).
%% {ok,#{outer_checksum => <<...>>,
%%       contents => [{"src/foo.erl",<<"-module(foo).">>}],
%%       metadata => #{<<"name">> => <<"foo">>, ...}}}
%%
%% > r3h_hex_tarball:unpack(Tarball, "path/to/unpack").
%% {ok,#{outer_checksum => <<...>>,
%%       metadata => #{<<"name">> => <<"foo">>, ...}}}
%% '''
...
unpack(Tarball, Output) ->
    case r3h_hex_erl_tar:extract({binary, Tarball}, [memory]) of
        {ok, []} ->
            {error, {tarball, empty}};
...
%%
%% ```
%% > r3h_hex_tarball:unpack_docs(Tarball, memory).
%% {ok, [{"index.html", <<"<!doctype>">>}, ...]}
%%
%% > r3h_hex_tarball:unpack_docs(Tarball, "path/to/unpack").
%% ok
%% '''
...
format_error({tarball, Reason}) -> "tarball error, " ++ r3h_hex_erl_tar:format_error(Reason);
format_error({inner_tarball, Reason}) -> "inner tarball error, " ++ r3h_hex_erl_tar:format_error(Reason);
...
format_error({metadata, Reason}) -> "error reading package metadata" ++ r3h_safe_erl_term:format_error(Reason);
...
do_decode_metadata(Binary) when is_binary(Binary) ->
    ...
    case r3h_safe_erl_term:string(String) of
        {ok, Tokens, _Line} ->
            try
                Terms = r3h_safe_erl_term:terms(Tokens),
        ...
        {error, {_Line, r3h_safe_erl_term, Reason}, _Line2} ->
...
unpack_tarball(ContentsBinary, memory) ->
    r3h_hex_erl_tar:extract({binary, ContentsBinary}, [memory, compressed]);
unpack_tarball(ContentsBinary, Output) ->
    filelib:ensure_dir(filename:join(Output, "*")),
    case r3h_hex_erl_tar:extract({binary, ContentsBinary}, [{cwd, Output}, compressed]) of
        ok ->
...
create_memory_tarball(Files) ->
    Path = tmp_path(),
    {ok, Tar} = r3h_hex_erl_tar:open(Path, [write]),

    try
        add_files(Tar, Files)
    after
        ok = r3h_hex_erl_tar:close(Tar)
    end,
...
add_file(Tar, {Filename, Contents}) when is_list(Filename) and is_binary(Contents) ->
    ok = r3h_hex_erl_tar:add(Tar, Contents, Filename, tar_opts());
...
add_file(Tar, {Filename, AbsFilename}) when is_list(Filename), is_list(AbsFilename) ->
    ...
    case FileInfo#file_info.type of
        symlink ->
            ok = r3h_hex_erl_tar:add(Tar, {Filename, AbsFilename}, tar_opts());
        directory ->
            case file:list_dir(AbsFilename) of
                {ok, []} ->
                    r3h_hex_erl_tar:add(Tar, {Filename, AbsFilename}, tar_opts());
        ...
        _ ->
            ...
            ok = r3h_hex_erl_tar:add(Tar, Contents, Filename, Mode, tar_opts())
    end.
