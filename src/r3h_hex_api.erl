...
-type response() :: {ok, {r3h_hex_http:status(), r3h_hex_http:headers(), body() | nil}} | {error, term()}.
...
request(Config, Method, Path, Body) when is_binary(Path) and is_map(Config) ->
    ...
    case r3h_hex_http:request(Config, Method, build_url(Path, Config), ReqHeaders2, Body) of
...
%% TODO: copy-pasted from r3h_hex_repo
make_headers(Config) ->
