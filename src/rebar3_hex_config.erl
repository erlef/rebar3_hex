...
%% @doc Strip the rebar-internal repo keys that are not part of
%% `hex_core:config()' so the resulting map is a valid config to pass to
%% `hex_cli_auth' / `hex_core'. Mirrors `rebar_packages:get_package_repo_config/1'.
-spec to_hex_config(map()) -> hex_core:config().
to_hex_config(Repo) ->
    maps:without([name, mirror_of, parent], Repo).
